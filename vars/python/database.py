...
class PostgresqlDatabase(Database):
    ...
    def move_to_product_discontinued_list(self, sku):
        cursor = self.connection.cursor()
        cursor.execute("INSERT INTO product_discontinued_list (sku) VALUES ('" + sku + "')")
        cursor.execute("DELETE FROM product_list WHERE sku = '" + sku + "'")
        self.connection.commit()

    def remove_discontinued_quote_price(self, sku):
        cursor = self.connection.cursor()
        cursor.execute("DELETE FROM quote_price WHERE product_id IN (SELECT product_id FROM product_list WHERE sku = '"
                       + sku + "')")
        self.connection.commit()

    def remove_expired_quote_price(self):
        cursor = self.connection.cursor()
        cursor.execute("DELETE FROM quote_price WHERE expiration_date < TIMESTAMP '" + datetime.datetime.now().strftime(
            '%Y-%m-%d %H:%M:%S') + "'")
        self.connection.commit()

    def remove_expired_list_price(self):
        cursor = self.connection.cursor()
        cursor.execute("DELETE FROM list_price WHERE expiration_date < TIMESTAMP '" + datetime.datetime.now().strftime(
            '%Y-%m-%d %H:%M:%S') + "'")
        self.connection.commit()

    def move_to_product_action_list_backup(self):
        cursor = self.connection.cursor()
        cursor.execute("INSERT INTO product_action_list_backup (sku, action, updated_date) "
                       + "SELECT sku, action, updated_date FROM product_action_list "
                       + "WHERE updated_date < NOW() - INTERVAL '3 MONTHS'")
